...
# Langtune Installer
# Installs Langtune with a dedicated virtual environment
...
INSTALL_DIR="$HOME/.langtrain/langtune"
BIN_DIR="$HOME/.local/bin"
VENV_DIR="$INSTALL_DIR/venv"

echo -e "\n${PRIMARY}⚡ Langtrain${SECONDARY} Tune${RESET}"
echo -e "${MUTED}Refining your installation environment...${RESET}\n"
...
# 4. Install
echo -e "${ACCENT}• Installing Langtune core...${RESET}"
"$VENV_DIR/bin/pip" install -U pip > /dev/null 2>&1
"$VENV_DIR/bin/pip" install -e . > /dev/null 2>&1 || "$VENV_DIR/bin/pip" install langtune --upgrade > /dev/null 2>&1
