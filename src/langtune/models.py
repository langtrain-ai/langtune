...
# =============================================================================
# Fast LoRA Language Model with Optimizations
# =============================================================================
...
class FastMultiHeadAttention(nn.Module):
    ...
    def __init__(
        ...
        if use_rope:
            try:
                from .optimizations import RotaryPositionEmbedding
                self.rotary_emb = RotaryPositionEmbedding(self.head_dim, max_seq_len)
            except ImportError:
                self.use_rope = False
                logger.warning("RoPE not available, falling back to standard position encoding")
        
        # Memory-efficient attention
        self.use_flash = use_flash_attention
        if use_flash_attention:
            try:
                from .optimizations import MemoryEfficientAttention
    ...
    def forward(
        ...
        for block in self.blocks:
            if self.use_gradient_checkpointing and self.training:
                try:
                    from .optimizations import checkpoint
                    x = checkpoint(self._forward_block, block, x, causal_mask)
                except ImportError:
        ...
        if labels is not None:
            ...
            try:
                from .optimizations import fused_cross_entropy
