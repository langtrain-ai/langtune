...
- [LLM Concepts Supported](#llm-concepts-supported)
...
## LLM Concepts Supported

Langtune provides stubs and extensibility for a variety of advanced LLM concepts and techniques:

| Concept | Full Name | Description |
|---------|-----------|-------------|
| RLHF    | Reinforcement Learning from Human Feedback | Fine-tuning models using human feedback, often via reward modeling and RL. |
| CoT     | Chain-of-Thought | Encourages models to generate intermediate reasoning steps for complex tasks. |
| CCoT    | Contrastive Chain-of-Thought | Uses contrastive learning to distinguish correct vs. incorrect reasoning chains. |
| GRPO    | Generalized Reinforcement Policy Optimization | Generalizes RL policy optimization methods like PPO and DPO. |
| RLVR    | Reinforcement Learning with Value Ranking | Uses value ranking to guide RL, often for preference-based optimization. |
| DPO     | Direct Preference Optimization | Aligns models directly with human preferences using pairwise comparisons. |
| PPO     | Proximal Policy Optimization | RL algorithm balancing exploration and exploitation with stable updates. |
| LIME    | Local Interpretable Model-agnostic Explanations | Explains model predictions by locally approximating with interpretable models. |
| SHAP    | SHapley Additive exPlanations | Explains model outputs using Shapley values from game theory. |

These concepts are available as stubs in the codebase and can be extended for research and experimentation.
